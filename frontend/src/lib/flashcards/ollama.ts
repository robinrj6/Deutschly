...
export async function generateExercisesWithOllama(words: ExerciseInput[]) {
  ...
  const prompt = [
    ...
    "Use a mix of: multiple_choice, fill_blank, true_false, word_scramble.",
    ...
    "Rules:",
    "- kind must be one of: multiple_choice, fill_blank, true_false, word_scramble",
    "- options must be 4 strings for multiple_choice",
    "- options must be ['True','False'] for true_false",
    "- options must be [] for fill_blank and word_scramble",
    "- answer must match the correct German word exactly",
    "- for true_false, answer must be 'True' or 'False'",
    "- question should be short",
    "- German grammar must be natural",
    "- For fill_blank: place _____ exactly where the target word naturally appears in German word order",
    "- Do NOT force every verb blank to sentence end",
    "- Put _____ at sentence end only when the structure requires it (e.g., modal + infinitive or subordinate clause)",
    "- In normal main clauses, finite verbs should follow normal V2 order",
    "- Bad example: 'Wir wollen _____ heute ...'",
    "- Good examples: 'Wir wollen heute ... _____.' and 'Heute _____ wir ...'",
